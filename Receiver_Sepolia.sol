...
/// @title - A receiver contract for receiving usdc tokens then calling a token swapper contract.
contract Receiver is CCIPReceiver, OwnerIsCreator {
    ...
    error InvalidTokenSwapper(); // Used when the token swapper address is 0
    ...
    error CallToTokenSwapperFailed(); // Used when the call to the stake function of the token swapper contract is not succesful
    error NoReturnDataExpected(); // Used if the call to the stake function of the token swapper contract returns data. This is not expected
    ...
    address private i_tokenSwapper;
    ...
    /// @param _tokenSwapper The address of the token swapper contract.
    constructor(
        address _router,
        address _usdcToken,
        address _tokenSwapper
    ) CCIPReceiver(_router) {
        if (_usdcToken == address(0)) revert InvalidUsdcToken();
        if (_tokenSwapper == address(0)) revert InvalidTokenSwapper();
        i_usdcToken = IERC20(_usdcToken);
        i_tokenSwapper = _tokenSwapper;
        i_usdcToken.safeApprove(_tokenSwapper, type(uint256).max);
    }


    function setTokenSwapperAddress(address tokenSwapperAddress) external {
        i_tokenSwapper = tokenSwapperAddress;
    }
    ...
    function _ccipReceive(
        ...
        (bool success, bytes memory returnData) = i_tokenSwapper.call(
            any2EvmMessage.data
        ); // low level call to the token swapper contract using the encoded function selector and arguments
        if (!success) revert CallToTokenSwapperFailed();
