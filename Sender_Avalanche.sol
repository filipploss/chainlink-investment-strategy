...
/// @title - A simple messenger contract for transferring tokens to a receiver  that calls a token swap @ Uniswap contract.
contract Sender is OwnerIsCreator {
    ...
    // Event to log incoming USDC deposits
    event USDCReceived(address indexed depositor, uint256 amount);
    ...
    AlpacaConnect private alpacaInstance;

    // TokenSwapper private tokenSwapperInstance;
    ...
    constructor(
        ...
        alpacaInstance = AlpacaConnect(_alpacaAddress);
    ...
    function buyStock(address _beneficiary, uint256 _amount) external {
        i_usdcToken.approve(address(this), _amount);
        string[] memory args;
        bytes32 result = alpacaInstance.sendRequest(
            i_functionsSubscriptionId,
            args
        );
        emit StockBought(_beneficiary, result, _amount);
    }
        ...
        returns (bytes32 messageId)
    {
        ...
        Client.EVM2AnyMessage memory evm2AnyMessage = Client.EVM2AnyMessage({
            ...
            data: abi.encodeWithSelector(
                ITokenSwap.swapTokens.selector,
                address(i_usdcToken),
                _tokenToBuy,
                _tokenToBuyUsdAmount
